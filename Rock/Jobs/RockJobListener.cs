        ...
        public void JobToBeExecuted( IJobExecutionContext context )
        {
            ...
            if ( job != null && job.Guid != Rock.SystemGuid.ServiceJob.JOB_PULSE.AsGuid() )
            {
                ...
                /* 
                     ...
                     After the job is executed, the ServiceJobHistory record's status, started, and stopped date times will be updated to match the job's last run.
                     
                     The job scheduler does not expose the job execution's actual start or stop time,
                     but it does expose the execution's run duration (in seconds) once the job is executed (available in the "JobWasExecuted" callback).
                     
                     In the "JobWasExecuted" callback, we update the ServiceJob.LastRunDurationSeconds value to the actual run duration returned by the scheduler,
                     and the ServiceJob.LastRunDateTime to the current system time. The last run start time is not stored in the ServiceJob.
